function [ first, second, third, fourth, rawdata, newraw, MultiRAID, measfile,test_single ] = JX_ReadSiemensMeasVD13_2DLCSSFP(measfile, isDynamicByAverage,dispopt)
...
% rawdata, - raw data in matrix; oversampled (x2) in complex (A+iB), indexed by loopcounters and
% ulChannelId;
...
display('POSSIBLE CHANGE OF DATA (double vs. single) ie EvalInfoMask(1) in particular ')
...
for iRAID = 1:NRAID
    ...
    fseek(fid, 0,'eof');
    FileSize = ftell(fid);
    Nbytes = FileSize - dumFtell ;
    
    % Changed for memory handling
%     [~, Nbytes] = fread(fid,inf,'uchar');
%     FileSize = ftell(fid);
...
%   cur_pos = readout_start;
    ...
    while 1
      ...
      % eval_info_mask.MDH_VOP                = min(bitand(sMDH.aulEvalInfoMask(2), 2^(53-32)),1);
%         eval_info_mask.MDH_VOP                = 0;
% 
%         eval_info_mask.MDH_IMASCAN            = 1;
%         if (eval_info_mask.MDH_ACQEND || eval_info_mask.MDH_RTFEEDBACK || eval_info_mask.MDH_HPFEEDBACK...
%                         || eval_info_mask.MDH_PHASCOR    || eval_info_mask.MDH_NOISEADJSCAN...
%                         || eval_info_mask.MDH_PHASESTABSCAN || eval_info_mask.MDH_REFPHASESTABSCAN...
%                         || eval_info_mask.MDH_SYNCDATA)
%             eval_info_mask.MDH_IMASCAN = 0; 
%         end
% 
%         if ( eval_info_mask.MDH_PATREFSCAN && ~eval_info_mask.MDH_PATREFANDIMASCAN )
%             eval_info_mask.MDH_IMASCAN = 0;
%         end
...
%          aADC(15) = mod(sCHA.ulChannelId, K)+1;
        ...
        %  aADC(35:35+MDH_NUMBEROFICEPROGRAMPARA-1) = sMDH.aushIceProgramPara;
           ...
           %%
           data = zeros(NADC, size(aADC,2),'single'); % Optimize to speed up!
           
           
           if isequal(aADC, double(single(aADC)))
               aADC  = single( aADC ) ;
           else
               test_single( iRAID ) = 1 ;
           end
...
%          aADC(15) = mod(sCHA.ulChannelId, K)+1;
    ...
    sMDH = sMDH1; % return the 1st sMDH
...
newraw=zeros(sMDH.sLC.ulChannelId,sMDH.sLC.ushPhase,loopcounters_max.ushLine,sMDH.ushSamplesInScan); %%2D
first = newraw;
second = newraw;
third = newraw;
fourth = newraw;
% newraw=zeros(sMDH.sLC.ushPartition,sMDH.sLC.ulChannelId,sMDH.sLC.ushPhase,loopcounters_max.ushLine,sMDH.ushSamplesInScan);%%
% 3D
% %% 

if isDynamicByAverage ==0
%%!OPTIONAL LOOPS! jsr
%for rep=1:1,%sMDH.sLC.ushRepetition,%4
    for measure = 1:sMDH.sLC.ushRepetition 
        newraw=zeros(sMDH.sLC.ulChannelId,sMDH.sLC.ushPhase,loopcounters_max.ushLine,sMDH.ushSamplesInScan); %%2D
        for phase=1:sMDH.sLC.ushPhase %10
            for coil=1:sMDH.sLC.ulChannelId %26
                for lc = 1:loopcounters_max.ushLine
                    lin_idx = find(loopcounters(:,1) == lc & loopcounters(:,2) == measure & loopcounters(:,6) == phase & loopcounters(:,15) == coil & loopcounters(:,16) == sMDH.ushSamplesInScan);                     
                    if numel(lin_idx)>=1
                        lin_idx = lin_idx(1);
                        newraw(coil,phase,lc,:)=rawdata(lin_idx,:);
                    end
                end 
            end    
        end
...
elseif isDynamicByAverage == 1
    for measure = 1:sMDH.sLC.ushAcquisition 
        newraw=zeros(sMDH.sLC.ulChannelId,sMDH.sLC.ushPhase,loopcounters_max.ushLine,sMDH.ushSamplesInScan); %%2D
        for phase=1:sMDH.sLC.ushPhase %10
            for coil=1:sMDH.sLC.ulChannelId %26
                for lc = 1:loopcounters_max.ushLine
                    lin_idx = find(loopcounters(:,3) == 1 & loopcounters(:,1) == lc & loopcounters(:,2) == measure & loopcounters(:,6) == phase & loopcounters(:,15) == coil & loopcounters(:,16) == sMDH.ushSamplesInScan);                     
                    %lin_idx = find( loopcounters(:,1) == lc & loopcounters(:,2) == measure & loopcounters(:,6) == phase & loopcounters(:,15) == coil & loopcounters(:,16) == sMDH.ushSamplesInScan);                     
                    if numel(lin_idx)>=1
                        lin_idx = lin_idx(1);
                        newraw(coil,phase,lc,:)=rawdata(lin_idx,:);
