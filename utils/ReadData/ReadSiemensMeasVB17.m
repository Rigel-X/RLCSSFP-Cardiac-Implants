function [rawdata,newraw,loopcounters,sMDH,lc_names] = ReadSiemensMeasVB17(measfile, dispopt)
...
%
% Maolin Qiu YALE 3-24-2011 (maolin.qiu(at)yale(dot)edu)
...
loopcounters = round(data(1:lcounter,1:IDX_DIM)+1); % all start from 1, unused -1
...
lc_names = {'sMDH.sLC.ushLine'
    'sMDH.sLC.ushAcquisition'
          'sMDH.sLC.ushSlice'
      'sMDH.sLC.ushPartition'
           'sMDH.sLC.ushEcho'
          'sMDH.sLC.ushPhase'
     'sMDH.sLC.ushRepetition'
            'sMDH.sLC.ushSet'
            'sMDH.sLC.ushSeg'
            'sMDH.sLC.ushIda'
            'sMDH.sLC.ushIdb'
...
disp(['[rawdata,loopcounters,sMDH] = ReadSiemensMeasVB17(measfile, dispopt);']);
disp(['See loopcounters for more information. Lines in rawdata are arranged']);
disp(['in the same order as in loopcounters']);
disp(['For details in MATLAB type help ReadSiemensMeasVB17']);
...
for lc = 1:loopcounters_max.ushLine
  ...
  case 3
    lin_idx = find(loopcounters(:,1)  == lc & loopcounters(:,15)  ==  8); % to see Ch = 8
     case 4
    lin_idx = find(loopcounters(:,1)  == lc & loopcounters(:,6)  ==  1); % to see phase 1
  otherwise
