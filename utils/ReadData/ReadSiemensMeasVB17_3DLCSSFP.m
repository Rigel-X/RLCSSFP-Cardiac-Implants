function [first, second, third, fourth, rawdata,newraw,loopcounters,sMDH,lc_names] = ReadSiemensMeasVB17_3DLCSSFP(measfile, dispopt)
...
%%!OPTIONAL LOOPS! jsr
    for rep=1:sMDH.sLC.ushPartition,%7
  ...
  lc
    ...
    lin_idx = find(loopcounters(:,1) == lc & loopcounters(:,4) == rep & loopcounters(:,7) == measure & loopcounters(:,6) == phase & loopcounters(:,15) == coil & loopcounters(:,16) == sMDH.ushSamplesInScan); 
...
%     newraw(part,coil,phase,lc,:)=rawdata(lin_idx,:);
    newraw(rep,coil,phase,lc,:)=rawdata(lin_idx,:);
 %   end
