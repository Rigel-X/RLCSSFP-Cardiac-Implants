function [first, second, third, fourth, rawdata,newraw,loopcounters,sMDH,lc_names] = ReadSiemensMeasVB17measures(measfile, dispopt)
%%!OPTIONAL ADD THE MATRICES IN OUTPUT IF CREATING MORE THAN ONE!! jsr
...
disp(['Length of the Header : ' num2str(hdr_len)]);
fseek(fid, hdr_len, 'bof'); %%..maolin modified according to Florian.Knoll at IDEA Board 4/30/2009
[tmp Nbytes] = fread(fid,inf,'uchar'); tmp = []; %JSR ADDED SKIP 10
FileSize = ftell(fid); 
...
%dcp adding in the "evilmask" info so that I dont get too much or too
%little data

%loopcounters_max.ushLine
%%%%%%%CHOSE FOR 3D OR 2D%%%%%%
...
%%!OPTIONAL LOOPS! jsr
%for rep=1:1,%sMDH.sLC.ushRepetition,%7
    for measure = 1:sMDH.sLC.ushRepetition; 
        newraw=zeros(sMDH.sLC.ulChannelId,sMDH.sLC.ushPhase,loopcounters_max.ushLine,sMDH.ushSamplesInScan); %%2D
%     for part = 1:sMDH.sLC.ushPartition;
    for phase=1:sMDH.sLC.ushPhase,%6
    for coil=1:sMDH.sLC.ulChannelId,%15
  phase
  coil
  ...
  lc
    ...
    lin_idx = find(loopcounters(:,1) == lc & loopcounters(:,7) == measure & loopcounters(:,6) == phase & loopcounters(:,15) == coil & loopcounters(:,16) == sMDH.ushSamplesInScan & loopcounters(:,4) == 1); 

    size(newraw)
    size(rawdata)
    lin_idx
    
 %   if lin_idx >= 1 && lin_idx <= length(rawdata),
%     newraw(part,coil,phase,lc,:)=rawdata(lin_idx,:);
